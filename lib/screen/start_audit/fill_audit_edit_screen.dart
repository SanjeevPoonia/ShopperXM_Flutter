...
class FillAuditEditScreen extends StatefulWidget {
  ...
  FillAuditEditScreen(this.questionList,this.parentList,this.storeID,this.cceName,this.customerCategoryName,this.scenarioID,this.auditMode,this.customerTypeID,this.contactNumber,this.beatPlanID,this.isVideoRequired,this.isImageRequired,this.isAudioRequired);
  @override
  MenuState createState() => MenuState();
}

class MenuState extends State<FillAuditEditScreen> {
    ...
    if(editEnabled)
      {
        for (int i = 0; i < currentQuestionList.length; i++) {
          radioSelectionList.add(currentQuestionList[i]["selected_option"]);
          imagePathList.add("");
          startTimeList.add(currentQuestionList[i]["start_time"].toString());
          endTimeList.add(currentQuestionList[i]["end_time"].toString());
          remarkList.add(TextEditingController(text:currentQuestionList[i]["remark"].toString()));
        }
      }
    else
      {
